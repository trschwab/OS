...
void *philosopher(void *args) {
    ...
    int right = i;
    int heldSticks = 0;
    while ((i < NUM_THREADS) || (heldSticks == 2)) {
        if (heldSticks == 2) {
            break;
        }
        if (pthread_mutex_lock(&locks[left]) == 0) {
            heldSticks++;
        }
        ...
        if (pthread_mutex_lock(&locks[(right+1) % NUM_THREADS]) == 0) { 
            heldSticks++;
        }
        printf("phila %d has %d stick\n", *thread, (right+1) % NUM_THREADS); 
        if (heldSticks == 2) {
            break;
        }
        i++;
    ...
    pthread_mutex_unlock(&locks[(right + 1) % NUM_THREADS]);
